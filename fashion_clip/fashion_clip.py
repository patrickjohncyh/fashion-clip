...
from fashion_clip.utils import (
    get_cache_directory,
    _is_hugging_face_repo,
    _model_processor_hash,
)
from fashion_clip.utils import (
    _download,
    file_sha256,
    display_images_from_s3,
    display_images_from_url,
    display_images,
)
...
class FCLIPDataset:
    ...
    def __init__(
        ...
        if name in _CATALOGS:
            ...
            raise RuntimeError(
                f"Catalog {name} not found; available catalogs = {list(_CATALOGS.keys())}"
            )
        self.name = name
        ...
        self.images_path = np.array(
            [os.path.join(image_source_path, _["image"]) for _ in catalog]
        )
    ...
    def display_products(
        self, ids: List[str], fields: Tuple[str] = ("id", "short_description"), **kwargs
    ):
        idxs = [self.id_to_idx[id] for id in ids]
        im_paths = list(self.images_path[idxs])
        prod_info = [
            {field: row[field] for field in fields} for row in self.catalog[idxs]
        ]
    ...
    def hash(self):
        ...
        return hashlib.sha256(
            (id_hash + images_hash + caption_hash).encode()
        ).hexdigest()
...
class FashionCLIP:
    ...
    def __init__(
        ...
        self.model, self.preprocess, self.model_hash = self._load_model(
            model_name, auth_token=auth_token
        )
        ...
        if normalize and dataset:
            ...
            self.image_vectors = self.image_vectors / np.linalg.norm(
                self.image_vectors, ord=2, axis=-1, keepdims=True
            )
            self.textual_vectors = self.textual_vectors / np.linalg.norm(
                self.textual_vectors, ord=2, axis=-1, keepdims=True
            )
            print("Done!")
    ...
    def _generate_vectors(self, cache=True):
        ...
        if cache:
            ...
            if not os.path.exists(os.path.join(_CACHE_DIR, "index.json")):
                with open(os.path.join(_CACHE_DIR, "index.json"), "w") as f:
                    json.dump(cache_index, f)
            else:
                with open(os.path.join(_CACHE_DIR, "index.json"), "r") as f:
                    cache_index = json.load(f)

            print("Checking cache...")
            if self.vector_hash in cache_index:
                print("Loading vectors from cache...")
                image_vectors_path = cache_index[self.vector_hash]["IMAGE"]
                text_vectors_path = cache_index[self.vector_hash]["TEXT"]
                image_vectors = np.load(image_vectors_path)
                textual_vectors = np.load(text_vectors_path)
                print(f"Loaded vectors from cache: {self.vector_hash}")
                return image_vectors, textual_vectors

            else:
                # check if dataset + model embedding exists in given cache
                if self.vector_hash in _VECTORS:
                    image_vectors_path = _download(
                        _VECTORS[self.vector_hash]["IMAGE"], _CACHE_DIR
                    )
                    text_vectors_path = _download(
                        _VECTORS[self.vector_hash]["TEXT"], _CACHE_DIR
                    )
                    image_vectors = np.load(image_vectors_path)
                    textual_vectors = np.load(text_vectors_path)

            print("No cache found, generating vectors...")
            ...
            print("Saving vectors to cache...")


            unique_cache_subdir = os.path.join(_CACHE_DIR, self.vector_hash)

            os.makedirs(unique_cache_subdir, exist_ok=True)

            image_vectors_path = os.path.join(
                unique_cache_subdir,
                f"{self.dataset.name}_IMAGE_VECTORS.npy",
            )

            text_vectors_path = os.path.join(
                unique_cache_subdir,
                f"{self.dataset.name}_TEXT_VECTORS.npy",
            )
            ...
            print(f"Saved vectors to cache: {self.vector_hash}")

            # save json reference to the vectors

            cache_index[self.vector_hash] = {
                "IMAGE": image_vectors_path,
                "TEXT": text_vectors_path,
            }
    ...
    def _load_model(
        ...
        if name in _MODELS or _is_hugging_face_repo(name, auth_token):
            ...
            preprocessing = CLIPProcessor.from_pretrained(
                name, use_auth_token=auth_token
            )
        ...
        # else it doesn't use HF, assume using OpenAI CLiP
            ...
            if os.path.isfile(name):
                ...
                raise RuntimeError(
                    f"Model {name} not found or not valid; available models = {list(_MODELS.keys())}"
                )

            model, preprocessing = clip.load(
                model_path, device=device, download_root=_CACHE_DIR
            )
    ...
    def encode_images(
        self, images: Union[List[str], List[PIL.Image.Image]], batch_size: int
    ):
        ImageFile.LOAD_TRUNCATED_IMAGES = True  # workaround for truncated images

        def transform_fn(el):
            imgs = (
                el["image"]
                if isinstance(el["image"][0], PIL.Image.Image)
                else [Image().decode_example(_) for _ in el["image"]]
            )
            return self.preprocess(images=imgs, return_tensors="pt")

        dataset = Dataset.from_dict({"image": images})
        dataset = (
            dataset.cast_column("image", Image(decode=False))
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                image_embeddings.extend(
                    self.model.get_image_features(**batch).detach().cpu().numpy()
                )
                pbar.update(1)
            pbar.close()
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                text_embeddings.extend(
                    self.model.get_text_features(**batch).detach().cpu().numpy()
                )
                pbar.update(1)
            pbar.close()
    ...
    def _cosine_similarity(
        self, key_vectors: np.ndarray, space_vectors: np.ndarray, normalize=True
    ):
        if normalize:
            key_vectors = key_vectors / np.linalg.norm(
                key_vectors, ord=2, axis=-1, keepdims=True
            )
        return np.matmul(key_vectors, space_vectors.T)

    def _nearest_neighbours(
        self, k, key_vectors, space_vectors, normalize=True, debug=False
    ):
        ...
        if self.approx:
            ...
            nn = [
                self.nn_index.get_nns_by_vector(
                    v, k, search_k=-1, include_distances=False
                )
            ...
            cosine_sim = self._cosine_similarity(
                key_vectors, space_vectors, normalize=normalize
            )
    ...
    def retrieval(self, queries: List[str], top_k: int = 10):
        ...
        return self._nearest_neighbours(
            k=top_k, key_vectors=text_vectors, space_vectors=self.image_vectors
        )
    ...
    def display_attention(self, image_path, query_text, pixel_size=15, iterations=5):
        heatmap, image = self._get_heatmap(
            image_path, query_text, pixel_size, iterations
        )
        attention_map.display_heatmap(image, query_text, heatmap)

    def _get_heatmap(self, image_path, text, pixel_size, iterations):
        images, masks = attention_map.generate_image_crops(
            image_path, pixel_size=pixel_size
        )
        ...
        image_vectors = image_vectors / np.linalg.norm(
            image_vectors, axis=-1, ord=2, keepdims=True
        )
